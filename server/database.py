...
class ChatDatabase:
    ...
    def init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Create fresh tables with user profile fields
            conn.execute("""
    ...
    def save_chat_session(self, session_name: str, messages: list, user_profile: dict = None):
        with sqlite3.connect(self.db_path) as conn:
            # Insert or update session with user profile
            if user_profile:
                conn.execute(
                    "INSERT OR REPLACE INTO chat_sessions (session_name, user_type, age_group, work_hours, updated_at) VALUES (?, ?, ?, ?, ?)",
                    (session_name, user_profile.get('user_type'), user_profile.get('age_group'), user_profile.get('work_hours'), datetime.now())
                )
            else:
                conn.execute(
    ...
    def save_user_profile(self, session_name: str, user_profile: dict):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT OR REPLACE INTO chat_sessions (session_name, user_type, age_group, work_hours, updated_at) VALUES (?, ?, ?, ?, ?)",
                (session_name, user_profile.get('user_type'), user_profile.get('age_group'), user_profile.get('work_hours'), datetime.now())
